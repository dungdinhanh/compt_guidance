...
do
  for skip in "${skips[@]}"
  do
cmd="python scripts_gdiff/compt_guidance/classifier_compt_sample_quadratic.py \
 ...
 --logdir runs/sampling_compt2/IMN256quad/unconditional/scale${scale}_skip${skip}/ \
...
cmd="python evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet256_labeled.npz \
 ${base_folder}/runs/sampling_compt2/IMN256quad/unconditional/scale${scale}_skip${skip}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
