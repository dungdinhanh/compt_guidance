...
#PBS -o output_256/compquad_im256cond_scale7p0_sk10.txt
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29456 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample_quadratic.py \
 ...
 --logdir runs/sampling_comptquad/IMN256/conditional/scale${scale}_skip${skip}/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet256_labeled.npz \
 ${base_folder}/runs/sampling_comptquad/IMN256/conditional/scale${scale}_skip${skip}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
