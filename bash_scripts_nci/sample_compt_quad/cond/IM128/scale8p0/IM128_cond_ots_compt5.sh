...
#PBS -o output_nci2/im128_compt_cond_quad_scale8p0_sk12.txt
#PBS -e errors/compt_cond_quad_scale2p0_error1.txt
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=49511 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample_quadratic.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/128x128_diffusion.pt $SAMPLE_FLAGS --classifier_path models/128x128_classifier.pt \
 --logdir runs/sampling_comptquad/IMN128/conditional/scale${scale}_skip${skip}/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet128_labeled.npz \
 ${base_folder}/runs/sampling_comptquad/IMN128/conditional/scale${scale}_skip${skip}/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
