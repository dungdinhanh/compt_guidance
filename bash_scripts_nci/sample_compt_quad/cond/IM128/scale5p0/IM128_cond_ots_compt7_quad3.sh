...
#PBS -o output_nci2/im128_compt_cond_quad_scale5p0_sk8_quad3.txt
#PBS -e errors/compt_cond_quad_scale2p0_error2.txt
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=49526 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample_general.py \
 ...
 --logdir runs/sampling_comptquad/IMN128/conditional_testquad/scale${scale}_skip${skip}_quad${quad}/ \
  --save_imgs_for_visualization True --base_folder ${base_folder} --skip ${skip} --k ${quad}"
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet128_labeled.npz \
 ${base_folder}/runs/sampling_comptquad/IMN128/conditional_testquad/scale${scale}_skip${skip}_quad${quad}/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
