...
#PBS -o output_nci/im64_compt_uncond_quad_sc11p0_sk9_10.txt
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29510 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample_quadratic.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --classifier_path models/64x64_classifier.pt \
 --logdir runs/sampling_compt2_quad/IMN64/unconditional/scale${scale}_skip${skip}/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt2_quad/IMN64/unconditional/scale${scale}_skip${skip}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
