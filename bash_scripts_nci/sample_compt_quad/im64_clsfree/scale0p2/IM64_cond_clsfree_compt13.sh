...
#PBS -o output_clsfree/comptquad64_clsfree_scale0p2_sk13.txt
#PBS -e errors/comptquad64_clsfree_scale0p1_error1.txt
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=30514 MARSV2_WHOLE_LIFE_STATE=0 python3  scripts_gdiff/compt_guidance/classifier_free_compt_sample_quadratic.py $MODEL_FLAGS  \
 --model_path models/64x64_diffusion.pt --uncond_model_path models/64x64_diffusion_unc.pt \
  $SAMPLE_FLAGS  --logdir runs/sampling_compt2/IMN64_clsfree_quad/conditional/scale${scale}_skip${skip}/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt2/IMN64_clsfree_quad/conditional/scale${scale}_skip${skip}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
