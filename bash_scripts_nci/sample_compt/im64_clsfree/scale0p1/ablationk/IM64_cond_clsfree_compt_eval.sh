...
#PBS -o output_ablation/compt64_clsfree_scale0p1_sk8_k1p3_eval2.txt
#PBS -e errors/compt64_clsfree_errors.txt
...
do
  ...
  do
    for k in "${ks[@]}"
    do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=30518 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_free_compt_sample_general.py $MODEL_FLAGS  \
 --model_path models/64x64_diffusion.pt --uncond_model_path models/64x64_diffusion_unc.pt \
  $SAMPLE_FLAGS  --logdir runs/sampling_compt2/IMN64_clsfree/conditional/scale${scale}_skip${skip}_k${k}/ \
  --save_imgs_for_visualization True --base_folder ${base_folder} --skip ${skip} --k ${k}"
#echo ${cmd}
#eval ${cmd}
done
done
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt2/IMN64_clsfree/conditional/scale${scale}_skip${skip}_k${k}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
