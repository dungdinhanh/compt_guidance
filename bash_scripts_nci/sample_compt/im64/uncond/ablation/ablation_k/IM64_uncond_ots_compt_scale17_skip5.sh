...
#PBS -o output_nci/compt_uncond_skip5_sc17_k1p2.txt
#PBS -e output_nci/compt_uncond_skip5_sc17_e1.txt
...
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=3003 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample.py $MODEL_FLAGS --classifier_scale ${scale}  \
 ...
 --logdir runs/sampling_compt2/IMN64/unconditional/scale${scale}_skip${skip}_k${k}/  --k ${k} \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt2/IMN64/unconditional/scale${scale}_skip${skip}_k${k}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
