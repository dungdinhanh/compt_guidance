...
#PBS -l jobfs=50GB
#PBS -l wd
#PBS -l storage=scratch/zg12+scratch/jp09
#PBS -M adin6536@uni.sydney.edu.au
#PBS -o output_nci/compt_uncond_es_skip1p3.txt
#PBS -e errors/es_skip.txt
...
base_folder="/scratch/jp09/dd9648/"
...
do
  for skip in "${skips[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29521 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample_es.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --classifier_path models/64x64_classifier.pt \
 --logdir runs/sampling_compt/IMN64/unconditional/scale${scale}_skip${skip}_es/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt/IMN64/unconditional/scale${scale}_skip${skip}_es/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
