...
#PBS -o output_nci2/compt_256uncond_log2.txt
#PBS -e output_nci2/compt_256uncond_error2.txt
...
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29550 MARSV2_WHOLE_LIFE_STATE=0 python3 scripts_gdiff/compt_guidance/classifier_compt_sample.py \
 ...
 --logdir runs/sampling_compt/IMN256/unconditional/scale${scale}_skip${skip}/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet256_labeled.npz \
 ${base_folder}/runs/sampling_compt/IMN256/unconditional/scale${scale}_skip${skip}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
