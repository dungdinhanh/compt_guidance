...
SAMPLE_FLAGS="--batch_size 55 --num_samples 55 --timestep_respacing 250 "
...
do
  for skip in "${skips[@]}"
  do
cmd="python scripts_gdiff/compt_guidance/classifier_compt_sample_noguidance_time.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/128x128_diffusion.pt $SAMPLE_FLAGS --classifier_path models/128x128_classifier.pt \
 --logdir runs/sampling_compt2t/IMN128/conditional/scale${scale}_skip${skip}/ \
