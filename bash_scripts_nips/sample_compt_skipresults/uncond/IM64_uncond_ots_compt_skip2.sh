...
cmd="python scripts_gdiff/compt_guidance/classifier_compt_sample.py $MODEL_FLAGS --classifier_scale ${scale}  \
 ...
 --logdir runs/sampling_compt2_skipinv/IMN64/unconditional/scale${scale}_skip${skip}/ \
...
cmd="python evaluations/evaluator_tolog.py ${base_folder}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${base_folder}/runs/sampling_compt2_skipinv/IMN64/unconditional/scale${scale}_skip${skip}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
