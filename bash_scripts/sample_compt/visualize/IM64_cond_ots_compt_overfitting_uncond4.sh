...
do
  for skip in "${skips[@]}"
  do
cmd="python scripts_gdiff/compt_guidance/analyse/visualize_path_classifier_compt_sample_overfitting4.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --classifier_path models/64x64_classifier.pt \
 --logdir runs/sampling_compt2quad_visualize_overfitting_imn64test2/IMN64_withxt_lthan200/unconditional/scale${scale}_skip${skip}/ \
