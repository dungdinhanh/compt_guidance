...
do
  for skip in "${skips[@]}"
  do
cmd="python scripts_gdiff/compt_guidance/analyse/visualize_path_classifier_compt_sample.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --model_path models/64x64_diffusion.pt $SAMPLE_FLAGS --classifier_path models/64x64_classifier.pt \
 --logdir runs/sampling_compt2_visualize/IMN64/conditional/scale${scale}_skip${skip}/ \
